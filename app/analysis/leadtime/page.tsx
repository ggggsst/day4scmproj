...
function GapCell({ row }: { row: LeadtimeGap }) {
  if (row.gap === null) return <EmptyValue value={null} reasonCode="CALCULATION_UNAVAILABLE" />;
  // 양수 = 실제가 마스터보다 길다 = 위험
  const tone = row.gap > 0 ? 'text-danger' : 'text-good';
  const sign = row.gap > 0 ? '+' : '';
  return <Badge status={row.gap > 0 ? 'WARNING' : 'SAFE'}>{`${sign}${formatNumber(row.gap, '일')}`}</Badge>;
}
...
const columns: DataColumn<LeadtimeGap>[] = [
  ...
  { key: 'masterLeadTime', label: '마스터', align: 'right',
    render: (r) => <EmptyValue value={formatNumber(r.masterLeadTime, '일')} reasonCode="CALCULATION_UNAVAILABLE" /> },
  { key: 'sampleCount', label: '표본수', align: 'right',
    render: (r) => r.sampleCount.toLocaleString() },
  { key: 'actualAverage', label: '실적평균', align: 'right',
    render: (r) => <EmptyValue value={formatNumber(r.actualAverage, '일')} reasonCode="CALCULATION_UNAVAILABLE" /> },
  { key: 'p80', label: 'P80', align: 'right',
    render: (r) => <EmptyValue value={formatNumber(r.p80, '일')} reasonCode="CALCULATION_UNAVAILABLE" /> },
  ...
  return (
    ...
    >
      <div className="grid grid-3">
        ...
        <KpiCard label="실제가 더 김" value={nLonger} foot="격차 > 0 인 공급처" tone="warning" />
        <KpiCard label="표본 부족" value={nLowSample} foot="표본 10건 미만" />
      </div>

      <Panel title="공급처별 리드타임" description="격차 = P80 − 마스터">
        <DataTable
          columns={columns}
          rows={rows}
